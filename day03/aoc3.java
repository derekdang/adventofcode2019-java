    ...
    public static void main(String[] args) {
        ...
        for (String s : split2) {
            ...
            if (s.startsWith("U")) {
                for(int x = 0; x < num; x++) {
                    ...
                    if (grid[i][j] == '|') {
                        grid[i][j] = 'X';
                        minSteps = Math.min(steps[i][j] + steps2, minSteps);
                    }
                }
            } else if (s.startsWith("R")) {
                for(int x = 0; x < num; x++) {
        ...
        for (i = 0; i < grid.length; i++) {
            for (j = 0; j < grid[i].length; j++) {
                int manhattanDist = Math.abs(CENTER-i) + Math.abs(CENTER-j);
                if (grid[i][j] == 'X' && (manhattanDist) < minDist ) {
                    minDist = manhattanDist;
                }
            }
        }

        // Answer for part 1: 2193
        System.out.printf("Manhattan distance of closest intersection to origin: %d\n", minDist);;

        // Answer for part 2: 63526
        System.out.printf("Fewest combined steps to reach an intersection of wires: %d\n", minSteps);
    }
}
